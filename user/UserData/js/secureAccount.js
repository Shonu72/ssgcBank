$(document).ready(function () {
  ...
  $("#NewUsername").blur(function (e) {
    ...
    if (flag == true) {
      ...
      $("#NewUserError").text(
        "Invalid Username Format ! Username can't start with number and sysmbol"
      );
      $("#NewUserError").css("color", "red");
      return false;
    }
      ...
      if (ConfirmPass == "") {
        ...
        $.ajax({
          ...
          success: function (response) {
            if (response == "success") {
              if (
                /^(?=.*\d)(?=.*[a-z])(?=.*[A-Z])(?!.* )(?=.*[^a-zA-Z0-9]).{8,16}$/m.test(
                  NewPass
                )
              ) {
                ...
                $("#NewPassError").text(
                  "Invalid Password ! Your password must be 8-12 characters long, contain letters and numbers, and must not contain spaces, special characters"
                );
                $("#NewPassError").css("color", "red");
              }
            } else {
